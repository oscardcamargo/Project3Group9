...
public:
    ...
    void resetMaze();
    // TODO: add and implement randomizedRecursiveDFS()
    void randomizedDFS();
...
void Maze::drawMaze()
{
    ...
    for (int i = 0; i <= _width; ++i)
    {
        ...
        {
            // TODO: draw the maze's solution

            if ((*_maze)[i][j].second)
            {
                hor[0] = sf::Vertex(sf::Vector2f(1+cell_dim*j, cell_dim*i), sf::Color::White);
                hor[1] = sf::Vertex(sf::Vector2f(1+cell_dim*j+cell_dim, cell_dim*i), sf::Color::White);
                texture.draw(hor, 2, sf::Lines);
            }

            if ((*_maze)[i][j].first)
            {
                ver[0] = sf::Vertex(sf::Vector2f(cell_dim*j, cell_dim*i), sf::Color::White);
                ver[1] = sf::Vertex(sf::Vector2f(1+cell_dim*j, 1+cell_dim*i+cell_dim), sf::Color::White);
